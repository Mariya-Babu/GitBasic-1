//Shortest Job First (SJF(Non-Preamptive)) Scheduling Algorithm implementation using C (Operating System Sheduling Algorithms)
#include<stdio.h>
...
int SJF_np(int*,int*,int*,int*,int,int);
...
int main(){
	int n;
	printf("SJF(Non-Preamptive) Algorithm implementation!");
...
int SJF_np(int* gc,int *bt, int* at, int* p, int k,int n){
	gc[n*0+0] = 0;
	gc[n*1+0]= -1;
	for(int i=0;i<n;i++){
		if(gc[n*0+k]<at[i]){
