// Priority(Preamptive) Scheduling Algorithm implementation using C (Operating System Sheduling Algorithms)
#include<stdio.h>
...
int Priority_p(int*,int*,int*,int*,int*,int,int,int);
...
int main(){
	int n;
	printf("Priority(Preamptive) Algorithm implementation!");
	printf("\nEnter no.of Processes : ");
	scanf("%d",&n);
	
	int n2 = 2*n;
	int bt[n], at[n],p[n],pr[n],gc[2][n2];  
	...
	//Gantt Chart Filling
	k = Priority_p(&gc[0][0],bt,at,p,pr,k,n,n2);
...
int Priority_p(int* gc,int *bt, int* at, int* p,int* pr, int k,int n,int n2){
	gc[n*0+0] = 0;
	gc[n*1+0]= -1;
	for(int i=0;i<n2;i++){
		{
