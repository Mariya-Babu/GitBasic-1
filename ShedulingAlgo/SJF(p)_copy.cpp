...
int main(){
	...
	printf("\n\nPrinting Waiting_times and Turn_around_times");
	printProcesses(pc,n);
	printArray(wt,n);
	printArray(tt,n);
...
//printing the table
void printTable(int *p, int *bt, int *at, int n){
	printf("\nPrinting the Table \n");
	printf("Pid  Brust Time   Arrival Time \n");
	for(int i=0;i<n;i++){
		printf("P%d	%d		%d",p[i]+1,bt[i],at[i]);
		printf("\n");
	}
}
